(function() {
  ...
  module.exports = Session = (function() {
	...
	Session.prototype.regenerate = function(cb){
      if(this.id && this.handler.cookie) {
        this.handler.cookie = new expressSession.Cookie(this.handler.cookie);
        let cookieSerialized = this.handler.cookie.serialize(this.req._appname, this.handler.cookie);
        this.req.res.setHeader('Set-Cookie', cookieSerialized);
        cb();
      }else
        cb();
    };
